...
# File to store verified truthfulness cases
DATABASE_FILE = "truthfulness_data.csv"

# Keyword lists for additional analysis
...
# Load past data or create an empty DataFrame
def load_database():
...
def analyze_with_openai(content, input_query):
    ...
    """
    try:
        # First Pass: Generate the initial analysis
        ...
        # Second Pass: Evaluate the initial response and finalize the truthfulness
...
def analyze_openai_response(response):
    ...
    # Extract the opening statement (first sentence)
    opening_statement = response.split('.')[0]

    # Core logic based on opening statement
    if "likely true" in opening_statement:
        ...
        score = 0.5  # Default to neutral if unclear

    # Look for contradictory keywords in the rest of the response
    if "misleading" in response or "not true" in response or "false" in response:
        score = min(score, 0.1)  # Lower the score if these phrases appear
    elif "accurate" in response or "clearly true" in response:
        score = max(score, 0.9)  # Raise the score if these phrases appear
...
def get_fact():
    # Example static facts (you can replace this with facts from a file)
    facts = [
...
@app.route('/about')
def about():
    return render_template('about.html')  # Render your about.html file
